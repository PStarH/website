...
const waitingMark = ref(0);

const escapedMark = ref(0);
...
const switchVal = ref(2);

const dfsPlayerPath = ref([]);

const arrival = (x, y) => {
    return x === props.rows - 1 && y === props.cols - 1;
}

const blockStyle = (x, y) => {
    return {
...
function handleFigurePosition(p) {
    switch (p) {
        case 0:
            grid.value = [
                [0, 2, 0, 0, 0, 0, 0],
...
function handleSwitchUpdate(value) {
    console.log(switchVal.value)
}

function handleSetBlock(x, y) {
    if (props.allowSetup) {
